...
每次打开都会明确提供两种选择：**免费模型（Kilo，免登录）** 或 **DeepSeek API**。默认路由是 Kilo Auto Free，使用 Kilo 的匿名免费额度，不使用本地模型；安装时会自动把它写入模型配置并选中，内置 Harness 的模型选择器还提供另一个 Kilo 免费模型。界面默认中文，用户之后仍可自行切换。选择 DeepSeek API 则使用原有的内置 DeepSeek key 配置流程。

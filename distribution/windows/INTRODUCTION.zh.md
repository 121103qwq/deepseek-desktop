...
- **免费模型（Kilo，免登录）**：默认使用 Kilo Auto Free 的匿名免费额度，不使用本地模型，也不要求登录或 API Key；内置“模型”设置还提供另一个 Kilo 免费模型。
- **DeepSeek API**：使用 DeepSeek Harness 的原有模型路由与内置 key 配置流程。

Release 提供一个后缀为 `Setup-默认.exe` 的标准安装包和一个带 `Offline` 名称的完整离线 `Setup.exe`。离线版同时内置固定版 WebView2 runtime。两者均为当前用户安装：默认程序文件位于 `%LOCALAPPDATA%\Programs\DeepSeek Desktop`，也可在安装向导中自定位置；会话和设置位于 `%LOCALAPPDATA%\DeepSeek Harness Data`，卸载程序保留会话和设置。
